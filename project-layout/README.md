...
![数据规范](./images/data_example.png)

上图是数据的规范，

- `DTO` - Data Transfer Object。在 API 层，通过序列化、反序列化的方式，将请求二进制转换成 DTO。

- `DO` - Domain Object。业务领域对象，包含业务逻辑。

- `PO` - Persistence Object。持久化对象，如果是数据库的话，那么就是数据库表的映射，ORM 的映射，不包含业务逻辑，但是可以包含简单的值校验。
